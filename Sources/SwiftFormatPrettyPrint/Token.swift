...
enum NewlineBehavior {
  /// Insert a single newline if the break fires.
  ///
  /// If `ignoresDiscretionary` is true, this break will not respect discretionary line breaks,
  /// regardless of the configuration setting `respectsExistingLineBreaks`. In other words, a
  /// newline will occur only if the break fires because the pretty printer determines that the line
  /// must break here.
  case `default`(ignoresDiscretionary: Bool = false)

  /// Insert the given number of soft newlines.
  ///
  /// If the total number of soft newlines that would be printed at a location would exceed the
  /// maximum allowed by the formatter configuration, only that maximum number will be printed.
  case soft(Int)

  /// Insert the given number of hard newlines.
  ///
  /// Hard newlines are never omitted by the pretty printer, even if it would result in a number of
  /// consecutive newlines that exceeds the limit allowed by the formatter configuration. This is
  /// used when representing content such as multiline string literals.
  case hard(Int)

  static let soft = NewlineBehavior.soft(1)

  static let hard = NewlineBehavior.hard(1)
}
...
//  }
  ...
  static let `break` = Token.break(.continue, size: 1, newlines: .default())

  static func `break`(_ kind: BreakKind, size: Int = 1) -> Token {
    return .break(kind, size: size, newlines: .default())
  }
